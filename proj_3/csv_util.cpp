 ...
 */
int append_image_data_csv(const char *filename, const char *image_filename, std::vector<float> &image_data,
                          int reset_file) {
    char buffer[256];
    char mode[8];
    FILE *fp;
    ...
    for (int i = 0; i < image_data.size(); i++) {
        char tmp[256];
        snprintf(tmp, sizeof(tmp), ",%.8f", image_data[i]);
 ...
 */
int read_image_data_csv(const char *filename, std::vector<char *> &filenames, std::vector<std::vector<float> > &data,
                        int echo_file) {
    FILE *fp;
    float fval;
    char img_file[256];
